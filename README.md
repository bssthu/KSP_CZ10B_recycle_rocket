...
- 0.4.0 在本机无测试标记启动到主菜单的生产冒烟测试通过：十个零件均由 PartLoader 编译，全局 `ERR=0`、`EXC=0`，且不会自动进入任务。
- 终端控制器的 250 组确定性扰动测试达到 250/250 进入捕获包线。
- 已生成并安装五个模板；`CZ10B kOS Hover Recovery Test` 与 `CZ10B Full Mission Recovery Test` 均已由 KSP 1.12.5 原生加载。
- 0.4.0 已在真实海面 Flight 场景完成一级点火、18 km 两级分离、boost-back、末段归中和四挂钩捕获；最终指标为垂直 −0.55 m/s、横向 1.46 m/s、倾角 1.8°，捕获后稳定到 `SEA_MISSION_TEST_PASS`，且仍余约 3.9 t 推进剂。
