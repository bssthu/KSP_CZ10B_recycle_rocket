...
Run333 已通过[强制约束与验收基线](mandatory-mission-constraints.md)的
自动逐项矩阵，等级为 `RECOVERED`：2 km 状态
`191.45 / 4.47 / 29.51`，随后实际进入严格提交集、四钩捕获、无触海并
完成 60.0 s 驻留。它不是 G-05N 名义通过，也不保证后续重播或改动自动
复现同一结果。

单独的`SEA_MISSION_TEST_PASS`字符串仍不是充分证据。每次结论都必须由
当轮逐项验收矩阵和人工视觉检查共同确认，并完成捕获后60 s保持；下面
的启动方式既用于复验成功，也用于发现回归。
...
11. 海拔2 km时下降速度150～200 m/s；分别检查`≤5 m/s / ≤10 m`名义质量和`≤10 m/s / ≤30 m`恢复准入。仅满足恢复准入时，确认位置追踪保持到实际进入`≤3.9 m/s / ≤8 m / ≤14.7°`严格提交集合；之后连续、近似竖直下降且不反弹。
