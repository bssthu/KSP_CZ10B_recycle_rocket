@LAZYGLOBAL OFF.
// Keep the execution rate used to tune ascent, entry, ballistic guidance and
// the main-ignition gate. The active save and every retained save backup
// explicitly record InstructionsPerUpdate=200; Runs 290--291 showed that
// guessing the documented default of 150 delays entry cutoff by about 0.8 s.
// Only the low-altitude safety phase switches to the higher rate below.
...
LOG MISSION_ID + ",CONTROL_CPU," + ROUND(TIME:SECONDS,3)
    + ",ipu=" + CONFIG:IPU
    TO "0:/cz10b/telemetry.csv".
...
LOCAL ENTRY_HOOK_OFFSET_FROM_COM IS BOOSTER_HOOK_OFFSET_FROM_COM().
...
LOCAL ENTRY_PREDICTED_MISS IS VXCL(ENTRY_UP,
        ENTRY_REL_POS - ENTRY_HOOK_OFFSET_FROM_COM)
...
UNTIL TERMINAL_IGNITION {
    ...
    LOCAL BALLISTIC_HOOK_OFFSET_FROM_COM IS
        BOOSTER_HOOK_OFFSET_FROM_COM().
    LOCAL BALLISTIC_H_POS IS VXCL(BALLISTIC_UP,
        BALLISTIC_REL_POS - BALLISTIC_HOOK_OFFSET_FROM_COM).
...
LOCAL PLAN_HOOK_OFFSET_FROM_COM IS BOOSTER_HOOK_OFFSET_FROM_COM().
...
LOCAL PLAN_H_POS IS VXCL(PLAN_UP,
    PLAN_REL_POS - PLAN_HOOK_OFFSET_FROM_COM).
...
LOCAL FINAL_ALIGN_TERMINAL_LOW_ENTRY_HOLD_CAPPED IS FALSE.
LOCAL FINAL_ALIGN_TERMINAL_EXTREME_LOW_PHASE_SHAPED IS FALSE.
...
LOCAL FINAL_CAPTURE_ATTITUDE_TARGET_INITIALIZED IS FALSE.
LOCAL FINAL_CAPTURE_ATTITUDE_PREVIOUS_TARGET IS
    SHIP:FACING:VECTOR:NORMALIZED.
LOCAL FINAL_CAPTURE_ATTITUDE_PREVIOUS_BODY_AXIS IS
    SHIP:FACING:VECTOR:NORMALIZED.
LOCAL FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE IS 0.
LOCAL FINAL_CAPTURE_ATTITUDE_PREVIOUS_ENERGY IS 0.
LOCAL FINAL_CAPTURE_ATTITUDE_GROWTH_DWELL IS 0.
...
UNTIL HOOK_CAPTURED() {
    ...
    // Run 288 timebase instrumentation: keep the ASL altitude on the exact
    // same loop-start clock as the guidance state. WRITE_TELEMETRY samples
    // TIME:SECONDS and SHIP:ALTITUDE later, which previously paired a new
    // altitude with old horizontal state.
    LOCAL CONTROL_SAMPLE_ALTITUDE IS SHIP:ALTITUDE.
    // Preserve the loop-count calibration of the complete formal trajectory,
    // then remove multi-frame computation delay before the 600 m attitude
    // monitor. The 200 m lead allows the next guidance update to start at the
    // high rate without moving the immutable 2 km acceptance state.
    IF CONTROL_SAMPLE_ALTITUDE <= 800 AND CONFIG:IPU < 2000 {
    ...
    LOCAL HOOK_OFFSET_FROM_COM IS BOOSTER_HOOK_OFFSET_FROM_COM().
    LOCAL HEIGHT IS -VDOT(REL_POS, UP_VEC).
    LOCAL HOOK_HEIGHT IS HEIGHT + BOOSTER_HOOK_OFFSET_ALONG_UP(UP_VEC).
    LOCAL COM_HORIZONTAL_POS IS VXCL(UP_VEC, REL_POS).
    LOCAL HORIZONTAL_HOOK_OFFSET IS VXCL(UP_VEC, HOOK_OFFSET_FROM_COM).
    LOCAL HORIZONTAL_POS IS COM_HORIZONTAL_POS
        - HORIZONTAL_HOOK_OFFSET.
    ...
    IF FINAL_ALIGN_MODE
        ...
        SET FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE TO CLAMP(
            ...
            FINAL_ALIGN_TERMINAL_AERO_BRAKE_MIN_HOLD_SCALE,
            1).
        IF FINAL_ALIGN_ENTRY_RATIO >= 0
            AND FINAL_ALIGN_ENTRY_RATIO
                <= FINAL_ALIGN_TERMINAL_LOW_ENTRY_HOLD_CAP_RATIO {
            SET FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE TO MIN(
                FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE,
                FINAL_ALIGN_TERMINAL_LOW_ENTRY_HOLD_CAP).
            SET FINAL_ALIGN_TERMINAL_LOW_ENTRY_HOLD_CAPPED TO TRUE.
        }
        IF FINAL_ALIGN_ENTRY_RATIO >= 0
            AND FINAL_ALIGN_ENTRY_RATIO
                <= FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_MAX_ENTRY_RATIO {
            LOCAL FINAL_ALIGN_TERMINAL_EXTREME_LOW_ACTIVE_HOLD_SCALE IS
                FINAL_ALIGN_TERMINAL_EXTREME_LOW_HOLD_SCALE
                + (FINAL_ALIGN_TERMINAL_EXTREME_LOW_HIGH_HOLD_SCALE
                    - FINAL_ALIGN_TERMINAL_EXTREME_LOW_HOLD_SCALE)
                    * CLAMP(
                        (FINAL_ALIGN_TERMINAL_ENTRY_RATIO
                            - FINAL_ALIGN_TERMINAL_EXTREME_LOW_HOLD_LOW_RATIO)
                        / MAX(
                            FINAL_ALIGN_TERMINAL_EXTREME_LOW_HOLD_HIGH_RATIO
                                - FINAL_ALIGN_TERMINAL_EXTREME_LOW_HOLD_LOW_RATIO,
                            0.001),
                        0, 1).
            SET FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE TO MIN(
                FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE,
                FINAL_ALIGN_TERMINAL_EXTREME_LOW_ACTIVE_HOLD_SCALE).
            SET FINAL_ALIGN_TERMINAL_EXTREME_LOW_PHASE_SHAPED TO TRUE.
        }
    ...
    LOCAL FINAL_ALIGN_TERMINAL_EXTREME_HIGH_POSITION_BIAS_BLEND IS CLAMP(
        ...
        / MAX(FINAL_ALIGN_TERMINAL_EXTREME_HIGH_POSITION_BIAS_FULL_RATIO
            - FINAL_ALIGN_TERMINAL_EXTREME_HIGH_POSITION_BIAS_RATIO,
            0.0001), 0, 1).
    LOCAL FINAL_ALIGN_TERMINAL_SCHEDULED_VELOCITY_COEFFICIENT IS
        TERMINAL_WAYPOINT_VELOCITY_COEFFICIENT
    ...
    LOCAL FINAL_ALIGN_TERMINAL_ACTIVE_VELOCITY_COEFFICIENT IS MAX(
        FINAL_ALIGN_TERMINAL_MIN_VELOCITY_COEFFICIENT,
        FINAL_ALIGN_TERMINAL_SCHEDULED_VELOCITY_COEFFICIENT).
    ...
    LOCAL FINAL_ALIGN_PRECOMMIT_ALONG_RANGE IS VDOT(
        HORIZONTAL_POS, CONTROL_ALONG_DIRECTION).
    ...
    LOCAL FINAL_ALIGN_PRECOMMIT_STOPPING_SURFACE IS
        ...
        - FINAL_ALIGN_PRECOMMIT_REACHABILITY_MARGIN.
    LOCAL FINAL_ALIGN_PRECOMMIT_RECTANGULAR_ENTRY IS
        HORIZONTAL_POS:MAG <= FINAL_ALIGN_PRECOMMIT_SETTLE_RANGE
        AND HORIZONTAL_VEL:MAG <= FINAL_ALIGN_PRECOMMIT_SETTLE_SPEED.
    LOCAL FINAL_ALIGN_PRECOMMIT_STANDARD_REACHABILITY_WINDOW IS
        FINAL_ALIGN_ENTRY_RATIO >= 0
        AND FINAL_ALIGN_ENTRY_RATIO
            <= FINAL_ALIGN_PRECOMMIT_REACHABILITY_MAX_ENTRY_RATIO
        AND HORIZONTAL_POS:MAG
            <= FINAL_ALIGN_PRECOMMIT_REACHABILITY_MAX_RANGE
        AND HORIZONTAL_VEL:MAG
            <= FINAL_ALIGN_PRECOMMIT_REACHABILITY_MAX_SPEED.
    LOCAL FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_REACHABILITY_WINDOW IS
        FINAL_ALIGN_ENTRY_RATIO >= 0
        AND FINAL_ALIGN_ENTRY_RATIO
            <= FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_MAX_ENTRY_RATIO
        AND HORIZONTAL_POS:MAG
            <= FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_MAX_RANGE
        AND HORIZONTAL_VEL:MAG
            <= FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_MAX_SPEED.
    LOCAL FINAL_ALIGN_PRECOMMIT_REACHABILITY_ENTRY IS
        (FINAL_ALIGN_PRECOMMIT_STANDARD_REACHABILITY_WINDOW
            OR FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_REACHABILITY_WINDOW)
        AND FINAL_ALIGN_PRECOMMIT_ALONG_CLOSING_SPEED > 0
        AND FINAL_ALIGN_PRECOMMIT_STOPPING_SURFACE <= 0.
    IF FINAL_ALIGN_MODE AND NOT FINAL_DESCENT_ARMED
        AND NOT FINAL_ALIGN_PRECOMMIT_SETTLE
        AND HOOK_HEIGHT > TERMINAL_WAYPOINT_HEIGHT
        AND (FINAL_ALIGN_PRECOMMIT_RECTANGULAR_ENTRY
            OR FINAL_ALIGN_PRECOMMIT_REACHABILITY_ENTRY)
        ...
        SET FINAL_ALIGN_PRECOMMIT_REACHABILITY_LATCHED TO
            FINAL_ALIGN_PRECOMMIT_REACHABILITY_ENTRY.
        SET FILTERED_H_ACCEL TO V(0,0,0).
        LOCAL FINAL_ALIGN_PRECOMMIT_ENTRY_REASON IS "rectangular".
        IF FINAL_ALIGN_PRECOMMIT_REACHABILITY_LATCHED {
            SET FINAL_ALIGN_PRECOMMIT_ENTRY_REASON TO "reachability".
            IF FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_REACHABILITY_WINDOW {
                SET FINAL_ALIGN_PRECOMMIT_ENTRY_REASON TO
                    "reachabilityExtremeLow".
            }
        }
        LOG MISSION_ID + ",FINAL_ALIGN_PRECOMMIT_SETTLE,"
            + ROUND(NOW,3)
            + ",reason=" + FINAL_ALIGN_PRECOMMIT_ENTRY_REASON
            + ",h=" + ROUND(HOOK_HEIGHT,2)
            + ",alongRange="
                + ROUND(FINAL_ALIGN_PRECOMMIT_ALONG_RANGE,3)
            + ",alongClosingSpeed="
                + ROUND(FINAL_ALIGN_PRECOMMIT_ALONG_CLOSING_SPEED,3)
            + ",stoppingDistance="
    ...
    IF FINAL_ALIGN_PRECOMMIT_REACHABILITY_LATCHED {
        SET FINAL_ALIGN_PRECOMMIT_ACTIVE_REACQUIRE_RANGE TO
            FINAL_ALIGN_PRECOMMIT_REACHABILITY_REACQUIRE_RANGE.
    }
    IF FINAL_ALIGN_PRECOMMIT_SETTLE AND NOT FINAL_DESCENT_ARMED
        AND HORIZONTAL_POS:MAG
            > FINAL_ALIGN_PRECOMMIT_ACTIVE_REACQUIRE_RANGE {
        SET FINAL_ALIGN_PRECOMMIT_SETTLE TO FALSE.
        SET FINAL_ALIGN_PRECOMMIT_REACHABILITY_LATCHED TO FALSE.
        SET FILTERED_H_ACCEL TO V(0,0,0).
    }
    LOCAL FINAL_ALIGN_PRECOMMIT_ACTIVE_MAX_ACCEL IS
        FINAL_ALIGN_PRECOMMIT_MAX_ACCEL.
    IF FINAL_ALIGN_PRECOMMIT_REACHABILITY_LATCHED {
        SET FINAL_ALIGN_PRECOMMIT_ACTIVE_MAX_ACCEL TO
            FINAL_ALIGN_PRECOMMIT_REACHABILITY_MAX_ACCEL.
    }
    ...
    IF NOT FINAL_CAPTURE_NEAR_NET_STEERING_TUNED
    ...
    // Once the committed trajectory enters the measured near-net capture
    // set, stop changing the position reference. The latch is one-way so a
    // centre crossing cannot regenerate a pursuit vector in the opposite
    // direction. Unlike attitude safe mode, active velocity damping remains.
    IF FINAL_DESCENT_ARMED
        AND NOT FINAL_CAPTURE_NEAR_NET_DAMPING_LATCHED
        AND HOOK_HEIGHT <= FINAL_CAPTURE_NEAR_NET_DAMPING_HEIGHT
        AND HORIZONTAL_POS:MAG <= FINAL_CAPTURE_NEAR_NET_DAMPING_RANGE
        AND HORIZONTAL_VEL:MAG <= FINAL_CAPTURE_NEAR_NET_DAMPING_SPEED {
        SET FINAL_CAPTURE_NEAR_NET_DAMPING_LATCHED TO TRUE.
        LOG MISSION_ID + ",FINAL_CAPTURE_NEAR_NET_DAMPING,"
            + ROUND(NOW,3)
            + ",h=" + ROUND(HOOK_HEIGHT,2)
            + ",range=" + ROUND(HORIZONTAL_POS:MAG,3)
            + ",speed=" + ROUND(HORIZONTAL_VEL:MAG,3)
    ...
    LOCAL FINAL_CAPTURE_ACTIVE_VELOCITY_GAIN IS
        FINAL_CAPTURE_VELOCITY_GAIN.
    IF FINAL_CAPTURE_NEAR_NET_DAMPING_LATCHED {
        SET FINAL_CAPTURE_ACTIVE_VELOCITY_GAIN TO
            FINAL_CAPTURE_NEAR_NET_DAMPING_VELOCITY_GAIN.
    }
    IF FINAL_DESCENT_ARMED {
        // Before entry to the near-net set, retain the bounded pursuit that
        // prevents accumulated aerodynamic drift. After entry, zero desired
        // velocity makes this branch a pure actively damped invariant set.
        LOCAL FINAL_CAPTURE_DESIRED_H_VEL IS V(0,0,0).
        LOCAL FINAL_CAPTURE_POSITION_RANGE IS HORIZONTAL_POS:MAG.
        IF NOT FINAL_CAPTURE_NEAR_NET_DAMPING_LATCHED
            AND FINAL_CAPTURE_POSITION_RANGE
                > FINAL_CAPTURE_POSITION_DEADBAND {
        ...
        SET H_ACCEL TO CLAMPV((FINAL_CAPTURE_DESIRED_H_VEL
            - HORIZONTAL_VEL) * FINAL_CAPTURE_ACTIVE_VELOCITY_GAIN,
    ...
    IF WAYPOINT_FINAL_COAST_MODE OR FINAL_DESCENT_ARMED
        OR HOOK_HEIGHT <= FINAL_CAPTURE_ATTITUDE_MONITOR_HEIGHT {
    ...
    LOCAL FINAL_ALIGN_TERMINAL_AERO_BRAKE_ACTIVE_FULL_HEIGHT IS
        FINAL_ALIGN_TERMINAL_AERO_BRAKE_TAIL_CEILING_FULL_HEIGHT.
    IF FINAL_ALIGN_TERMINAL_EXTREME_LOW_PHASE_SHAPED {
        SET FINAL_ALIGN_TERMINAL_AERO_BRAKE_ACTIVE_FULL_HEIGHT TO
            FINAL_ALIGN_TERMINAL_EXTREME_LOW_TAIL_CEILING_FULL_HEIGHT.
    }
    LOCAL FINAL_ALIGN_TERMINAL_AERO_BRAKE_TAIL_CEILING_BLEND IS CLAMP(
        ...
        / MAX(FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_END_HEIGHT
            - FINAL_ALIGN_TERMINAL_AERO_BRAKE_ACTIVE_FULL_HEIGHT,
            1), 0, 1).
    ...
    LOCAL FINAL_ALIGN_PRECOMMIT_ACTIVE_AERO_BRAKE_TAIL_CEILING IS
        FINAL_ALIGN_PRECOMMIT_REACHABILITY_AERO_BRAKE_TAIL_CEILING.
    IF FINAL_ALIGN_PRECOMMIT_REACHABILITY_LATCHED
        AND FINAL_ALIGN_ENTRY_RATIO >= 0
        AND FINAL_ALIGN_ENTRY_RATIO
            <= FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_MAX_ENTRY_RATIO {
        SET FINAL_ALIGN_PRECOMMIT_ACTIVE_AERO_BRAKE_TAIL_CEILING TO
            FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_AERO_BRAKE_TAIL_CEILING.
    }
    IF FINAL_ALIGN_PRECOMMIT_REACHABILITY_LATCHED {
        LOCAL FINAL_ALIGN_PRECOMMIT_REACHABILITY_TAIL_CEILING IS
            FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE
            + (MAX(FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE,
                    FINAL_ALIGN_PRECOMMIT_ACTIVE_AERO_BRAKE_TAIL_CEILING)
                - FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_SCALE)
                * FINAL_ALIGN_TERMINAL_AERO_BRAKE_TAIL_CEILING_BLEND.
        SET FINAL_ALIGN_TERMINAL_AERO_BRAKE_TAIL_CEILING TO MAX(
    ...
    IF FINAL_ALIGN_TERMINAL_EXTREME_LOW_PHASE_SHAPED
        ...
        AND HOOK_HEIGHT > TERMINAL_WAYPOINT_HEIGHT
        AND VDOT(HORIZONTAL_POS, CONTROL_ALONG_DIRECTION) > 0
        AND MAIN_ALONG_VEL > TERMINAL_ALONG_AERO_BRAKE_RELEASE_SPEED {
        LOCAL FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR_BLEND IS CLAMP(
            (FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR_START_HEIGHT
                - HOOK_HEIGHT)
            / MAX(
                FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR_START_HEIGHT
                ...
                1),
            0, 1).
        SET FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR TO
            FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR_START_BLEND
            + (1
                - FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR_START_BLEND)
                * FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR_BLEND.
        SET ALONG_AERO_BRAKE_BLEND_STATE TO MAX(
            ALONG_AERO_BRAKE_BLEND_STATE,
            TERMINAL_ALONG_AERO_BRAKE_MAX_BLEND
                * MIN(
    ...
    // Low-altitude reference governor and stability monitor. The governor
    // bounds the exogenous command rate before it reaches the nested kOS
    // direction/rate/torque loops. The monitor then measures the physical
    // closed-loop state, not controller output: pointing error plus filtered
    // transverse rate form an energy-like envelope. Sustained growth catches
    // divergent ringing while a separate hard-rate edge catches a fast loss.
    LOCAL FINAL_CAPTURE_ATTITUDE_MONITOR_ACTIVE IS
        HOOK_HEIGHT <= FINAL_CAPTURE_ATTITUDE_MONITOR_HEIGHT.
    LOCAL FINAL_CAPTURE_ATTITUDE_CONE_BARRIER_TRIGGERED IS
        FINAL_CAPTURE_ATTITUDE_MONITOR_ACTIVE
    ...
    LOCAL FINAL_CAPTURE_ATTITUDE_ERROR IS 0.
    LOCAL FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE IS 0.
    LOCAL FINAL_CAPTURE_ATTITUDE_REPORTED_TOTAL_RATE IS 0.
    LOCAL FINAL_CAPTURE_ATTITUDE_ENERGY IS 0.
    ...
    IF FINAL_CAPTURE_ATTITUDE_SAFE_MODE
        ...
        SET FINAL_CAPTURE_ATTITUDE_EFFECTIVE_COMMAND_CONE_DEGREES TO
            FINAL_CAPTURE_ATTITUDE_SAFE_COMMAND_CONE_DEGREES.
    }
    IF FINAL_CAPTURE_ATTITUDE_MONITOR_ACTIVE {
        ...
        // Run262 measured a 0.64 s guidance cadence: using capped DT reduced a
        // configured 3 deg/s governor to only about 0.47 deg/s.
        LOCAL FINAL_CAPTURE_ATTITUDE_MAX_TARGET_STEP IS
            FINAL_CAPTURE_ATTITUDE_TARGET_RATE_LIMIT
            * POWERED_MEASUREMENT_DT.
        ...
        // onto SHIP:FACING mislabeled axial roll as transverse motion in this
        // craft/control frame: kOS reported about 10 deg/s transverse while
        // the independent Unity-frame observer measured about 10 deg/s roll
        // and only 1--3 deg/s pitch/yaw. The longitudinal body axis does not
        // move under pure roll, so its finite-difference rate is a
        // frame-invariant transverse estimate.
        IF FINAL_CAPTURE_ATTITUDE_WAS_INITIALIZED {
            SET FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE TO VANG(
                FINAL_CAPTURE_ATTITUDE_PREVIOUS_BODY_AXIS,
                FINAL_CAPTURE_ATTITUDE_BODY_AXIS)
                / MAX(POWERED_MEASUREMENT_DT, 0.001).
        }
        LOCAL FINAL_CAPTURE_ATTITUDE_FILTER_BLEND IS
            POWERED_MEASUREMENT_DT
            / (FINAL_CAPTURE_ATTITUDE_RATE_FILTER_SECONDS
                + POWERED_MEASUREMENT_DT).
        IF NOT FINAL_CAPTURE_ATTITUDE_WAS_INITIALIZED {
            SET FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE TO
                FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE.
        } ELSE {
            SET FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE TO
                FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE
                    * (1 - FINAL_CAPTURE_ATTITUDE_FILTER_BLEND)
                + FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE
                    * FINAL_CAPTURE_ATTITUDE_FILTER_BLEND.
        }
        SET FINAL_CAPTURE_ATTITUDE_ENERGY TO
            (FINAL_CAPTURE_ATTITUDE_ERROR
                / FINAL_CAPTURE_ATTITUDE_ENERGY_ERROR_SCALE)^2
            + (FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE
                / FINAL_CAPTURE_ATTITUDE_ENERGY_RATE_SCALE)^2.
        IF FINAL_CAPTURE_ATTITUDE_WAS_INITIALIZED {
            SET FINAL_CAPTURE_ATTITUDE_ENERGY_GROWTH_RATE TO
                (FINAL_CAPTURE_ATTITUDE_ENERGY
                    - FINAL_CAPTURE_ATTITUDE_PREVIOUS_ENERGY)
                / MAX(POWERED_MEASUREMENT_DT, 0.001).
        }
        LOCAL FINAL_CAPTURE_ATTITUDE_GROWTH_QUALIFIES IS
            FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE
                >= FINAL_CAPTURE_ATTITUDE_WARNING_RATE
            AND FINAL_CAPTURE_ATTITUDE_ERROR
                >= FINAL_CAPTURE_ATTITUDE_WARNING_ERROR
            AND FINAL_CAPTURE_ATTITUDE_ENERGY_GROWTH_RATE
                >= FINAL_CAPTURE_ATTITUDE_ENERGY_GROWTH_PER_SECOND.
        IF FINAL_CAPTURE_ATTITUDE_GROWTH_QUALIFIES {
            SET FINAL_CAPTURE_ATTITUDE_GROWTH_SAMPLE_COUNT TO
                FINAL_CAPTURE_ATTITUDE_GROWTH_SAMPLE_COUNT + 1.
            // The first qualifying endpoint does not prove persistence over
            // the preceding interval. Begin dwell only after a second
            // consecutive sample confirms the same growing mode.
            IF FINAL_CAPTURE_ATTITUDE_GROWTH_SAMPLE_COUNT >= 2 {
                SET FINAL_CAPTURE_ATTITUDE_GROWTH_DWELL TO
                    FINAL_CAPTURE_ATTITUDE_GROWTH_DWELL
                    + POWERED_MEASUREMENT_DT.
            }
        } ELSE {
            SET FINAL_CAPTURE_ATTITUDE_GROWTH_SAMPLE_COUNT TO 0.
            SET FINAL_CAPTURE_ATTITUDE_GROWTH_DWELL TO 0.
        }
        IF NOT FINAL_CAPTURE_ATTITUDE_WARNING_LOGGED
            AND FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE
                >= FINAL_CAPTURE_ATTITUDE_WARNING_RATE {
            SET FINAL_CAPTURE_ATTITUDE_WARNING_LOGGED TO TRUE.
            LOG MISSION_ID + ",FINAL_CAPTURE_ATTITUDE_WARNING,"
                + ROUND(NOW,3)
                + ",h=" + ROUND(HOOK_HEIGHT,2)
                + ",error=" + ROUND(FINAL_CAPTURE_ATTITUDE_ERROR,3)
                + ",rawRate="
                    + ROUND(FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE,3)
                + ",filteredRate="
                    + ROUND(
                        FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE,3)
                + ",energy=" + ROUND(FINAL_CAPTURE_ATTITUDE_ENERGY,3)
                TO "0:/cz10b/telemetry.csv".
        }
        LOCAL FINAL_CAPTURE_ATTITUDE_TRIGGER_REASON IS "".
        IF FINAL_CAPTURE_ATTITUDE_CONE_BARRIER_TRIGGERED {
            SET FINAL_CAPTURE_ATTITUDE_TRIGGER_REASON TO "coneBarrier".
        } ELSE IF FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE
                >= FINAL_CAPTURE_ATTITUDE_HARD_RATE
            AND ACTUAL_CONE_GUARD_ANGLE
                >= FINAL_CAPTURE_ATTITUDE_HARD_RATE_MIN_ACTUAL_CONE {
            SET FINAL_CAPTURE_ATTITUDE_TRIGGER_REASON TO "hardRate".
        } ELSE IF FINAL_CAPTURE_ATTITUDE_GROWTH_DWELL
                >= FINAL_CAPTURE_ATTITUDE_GROWTH_DWELL_SECONDS
            AND FINAL_CAPTURE_ATTITUDE_GROWTH_SAMPLE_COUNT >= 2
            AND ACTUAL_CONE_GUARD_ANGLE
                >= FINAL_CAPTURE_ATTITUDE_ENERGY_MIN_ACTUAL_CONE {
            SET FINAL_CAPTURE_ATTITUDE_TRIGGER_REASON TO "energyGrowth".
        }
        IF NOT FINAL_CAPTURE_ATTITUDE_SAFE_MODE
            AND FINAL_CAPTURE_ATTITUDE_TRIGGER_REASON <> "" {
            SET FINAL_CAPTURE_ATTITUDE_SAFE_MODE TO TRUE.
            SET STEERINGMANAGER:MAXSTOPPINGTIME TO
                FINAL_CAPTURE_ATTITUDE_SAFE_MAX_STOPPING_TIME.
            STEERINGMANAGER:RESETPIDS().
            LOG MISSION_ID + ",FINAL_CAPTURE_ATTITUDE_SAFE_MODE,"
                + ROUND(NOW,3)
                + ",reason=" + FINAL_CAPTURE_ATTITUDE_TRIGGER_REASON
                + ",h=" + ROUND(HOOK_HEIGHT,2)
                + ",error=" + ROUND(FINAL_CAPTURE_ATTITUDE_ERROR,3)
                + ",rawRate="
                    + ROUND(FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE,3)
                ...
                + ",energy=" + ROUND(FINAL_CAPTURE_ATTITUDE_ENERGY,3)
                + ",actualCone="
                    + ROUND(ACTUAL_CONE_GUARD_ANGLE,3)
                + ",coneMargin=" + ROUND(
                    TERMINAL_VELOCITY_CONE_DEGREES
                        - ACTUAL_CONE_GUARD_ANGLE,3)
                TO "0:/cz10b/telemetry.csv".
        }
        IF FINAL_CAPTURE_ATTITUDE_SAFE_MODE {
            SET FINAL_CAPTURE_ATTITUDE_EFFECTIVE_COMMAND_CONE_DEGREES TO
        ...
        SET FINAL_CAPTURE_ATTITUDE_PREVIOUS_TARGET TO STEERING_THRUST.
        SET FINAL_CAPTURE_ATTITUDE_PREVIOUS_BODY_AXIS TO
            FINAL_CAPTURE_ATTITUDE_BODY_AXIS.
    ...
    // Measure actual state-to-command computation latency independently of
    // the large diagnostic string constructed below.
    LOCAL CONTROL_APPLY_UT IS TIME:SECONDS.
    LOCAL CONTROL_APPLY_DELAY IS CONTROL_APPLY_UT - NOW.
    ...
    IF NOW - LAST_LOG >= TELEMETRY_PERIOD {
        ...
        LOCAL DIAG_REPORTED_TOTAL_ANGULAR_RATE_DEG IS
            SHIP:ANGULARVEL:MAG * CONSTANT:RADTODEG.
        LOCAL DIAG_TRANSVERSE_ANGULAR_RATE_DEG IS
            FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE.
        LOCAL DIAG_AXIAL_ANGULAR_RATE_DEG IS SQRT(MAX(
            DIAG_REPORTED_TOTAL_ANGULAR_RATE_DEG^2
                - DIAG_TRANSVERSE_ANGULAR_RATE_DEG^2, 0)).
        ...
        LOG MISSION_ID + ",GUIDANCE_VECTOR," + ROUND(NOW,3)
            + ",h=" + ROUND(HOOK_HEIGHT,2)
            + ",sampleAltitude=" + ROUND(CONTROL_SAMPLE_ALTITUDE,2)
            + ",controlApplyUt=" + ROUND(CONTROL_APPLY_UT,3)
            + ",controlApplyDelay=" + ROUND(CONTROL_APPLY_DELAY,3)
            ...
            + ",finalAlignTerminalLowEntryHoldCapped="
                + FINAL_ALIGN_TERMINAL_LOW_ENTRY_HOLD_CAPPED
            + ",finalAlignTerminalExtremeLowPhaseShaped="
                + FINAL_ALIGN_TERMINAL_EXTREME_LOW_PHASE_SHAPED
            + ",finalAlignTerminalAeroBrakeHoldEndHeight="
                + ROUND(
                    FINAL_ALIGN_TERMINAL_AERO_BRAKE_HOLD_END_HEIGHT,1)
            + ",finalAlignTerminalAeroBrakeFullHeight="
                + ROUND(
                    FINAL_ALIGN_TERMINAL_AERO_BRAKE_ACTIVE_FULL_HEIGHT,1)
            + ",finalAlignTerminalAeroBrakeTailCeiling="
                + ROUND(
                    FINAL_ALIGN_TERMINAL_AERO_BRAKE_TAIL_CEILING,3)
            + ",finalAlignTerminalExtremeLowLateFloor="
                + ROUND(
                    FINAL_ALIGN_TERMINAL_EXTREME_LOW_LATE_FLOOR,3)
            ...
            + ",comAlongPos="
                + ROUND(VDOT(
                    COM_HORIZONTAL_POS, CONTROL_ALONG_DIRECTION),3)
            + ",hookAlongOffset="
                + ROUND(VDOT(
                    HORIZONTAL_HOOK_OFFSET,
                    CONTROL_ALONG_DIRECTION),3)
            ...
            + ",finalAlignPrecommitExtremeLowWindow="
                + FINAL_ALIGN_PRECOMMIT_EXTREME_LOW_REACHABILITY_WINDOW
            + ",finalAlignPrecommitStoppingDistance="
                + ROUND(FINAL_ALIGN_PRECOMMIT_STOPPING_DISTANCE,3)
            ...
            + ",finalAlignPrecommitActiveAeroBrakeTailCeiling="
                + ROUND(
                    FINAL_ALIGN_PRECOMMIT_ACTIVE_AERO_BRAKE_TAIL_CEILING,3)
            ...
            + ",activeConeLimit="
                + ROUND(
                    FINAL_CAPTURE_ATTITUDE_EFFECTIVE_COMMAND_CONE_DEGREES,2)
            ...
            + ",reportedTotalAngularRate="
                + ROUND(DIAG_REPORTED_TOTAL_ANGULAR_RATE_DEG,3)
            ...
            + ",attitudeMonitorActive="
                + FINAL_CAPTURE_ATTITUDE_MONITOR_ACTIVE
            + ",attitudeConeBarrier="
                + FINAL_CAPTURE_ATTITUDE_CONE_BARRIER_TRIGGERED
            + ",attitudeConeMargin=" + ROUND(
                TERMINAL_VELOCITY_CONE_DEGREES
                    - ACTUAL_CONE_GUARD_ANGLE,3)
            ...
            + ",attitudeAppliedTargetRate="
                + ROUND(FINAL_CAPTURE_ATTITUDE_APPLIED_TARGET_RATE,3)
            ...
            + ",attitudeRawTransverseRate="
                + ROUND(
                    FINAL_CAPTURE_ATTITUDE_RAW_TRANSVERSE_RATE,3)
            + ",attitudeReportedTotalRate="
                + ROUND(FINAL_CAPTURE_ATTITUDE_REPORTED_TOTAL_RATE,3)
            + ",attitudeFilteredTransverseRate="
                + ROUND(
                    FINAL_CAPTURE_ATTITUDE_FILTERED_TRANSVERSE_RATE,3)
            + ",attitudeEnergy="
                + ROUND(FINAL_CAPTURE_ATTITUDE_ENERGY,3)
            + ",attitudeEnergyGrowthRate="
                + ROUND(
                    FINAL_CAPTURE_ATTITUDE_ENERGY_GROWTH_RATE,3)
            + ",attitudeGrowthDwell="
                + ROUND(FINAL_CAPTURE_ATTITUDE_GROWTH_DWELL,3)
            + ",attitudeGrowthSamples="
                + FINAL_CAPTURE_ATTITUDE_GROWTH_SAMPLE_COUNT
            + ",attitudeSafeMode="
                + FINAL_CAPTURE_ATTITUDE_SAFE_MODE
        ...
        // Capture the end of both telemetry writes before constructing this
        // compact timing row. This reveals how much of the loop period is
        // control computation versus diagnostic serialization.
        LOCAL TELEMETRY_COMPLETE_UT IS TIME:SECONDS.
        LOG MISSION_ID + ",CONTROL_TIMING," + ROUND(NOW,3)
            + ",sampleAltitude=" + ROUND(CONTROL_SAMPLE_ALTITUDE,2)
            + ",ipu=" + CONFIG:IPU
            + ",controlApplyUt=" + ROUND(CONTROL_APPLY_UT,3)
            + ",controlApplyDelay=" + ROUND(CONTROL_APPLY_DELAY,3)
            + ",telemetryCompleteUt="
                + ROUND(TELEMETRY_COMPLETE_UT,3)
            + ",telemetryCompleteDelay="
                + ROUND(TELEMETRY_COMPLETE_UT - NOW,3)
            TO "0:/cz10b/telemetry.csv".
        SET LAST_LOG TO NOW.
    }
